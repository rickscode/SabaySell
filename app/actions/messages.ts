...
export async function sendMessage(
  ...
  try {
    ...
    if (updateThreadError) {
      console.error('Error updating thread:', updateThreadError)
    }
...
export async function createThreadAndSendMessage(
  ...
  try {
    ...
    const { thread, isNew } = await findOrCreateThread(listingId, user.id)
    ...
    console.error('Unexpected error creating thread and sending message:', error)
    return { success: false, error: 'Unexpected error occurred' }
  }
}
